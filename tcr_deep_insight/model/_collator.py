...
class AminoAcidsCollator:
    ...
    def _rand_mask(self, input_ids, attention_mask) -> torch.Tensor:
        ...
        mask_idx = list(map(lambda z:z[0], list(filter(lambda x: x[1], enumerate(attention_mask)))))
    ...
    def __call__(self, input_ids, attention_mask) -> torch.Tensor:
        if len(input_ids.shape) == 1:
            ...
            ret = [self._rand_mask(i, a) for i,a in zip(input_ids, attention_mask)]
            return torch.vstack(
                list(map(lambda x: x[0], ret))
            ), torch.vstack(
...
class TRABCollator:
    def __init__(
        ...
        tra_max_length:int,
        trb_max_length:int,
        mask_token_id: int = _AMINO_ACIDS_INDEX[_AMINO_ACIDS_ADDITIONALS["MASK"]],
    ...
    def _rand_mask(self, input_ids, attention_mask) -> Tuple[torch.Tensor]:
        ...
        if mask_tr < self.mask_trb_probability:
            ...
            mask_idx = list(map(lambda z: z[0], list(filter(lambda x: ((_AMINO_ACIDS_INDEX_REVERSE.get(x[1][0], None) in _AMINO_ACIDS) or x[1][0] > len(_AMINO_ACIDS_INDEX_REVERSE)) and x[1][1], enumerate(zip(ids, mask))))))

            if not mask_idx:
                return torch.tensor(input_ids, dtype=torch.int64), torch.tensor(attention_mask, dtype=torch.bool)

            n_mask = int(len(mask_idx) * self.mlm_probability)
            _mask_idx = np.random.choice(mask_idx, n_mask)

            if ids[mask_idx[0]] in VJ_GENES2INDEX.values() and mask_idx[0] not in _mask_idx:
                _mask_idx = np.hstack([mask_idx[0], _mask_idx])
            ids[_mask_idx] = self.mask_token_id
            mask[_mask_idx] = False

            return torch.tensor(np.hstack([input_ids[: self.tra_max_length], ids]), dtype=torch.int64), torch.tensor(np.hstack([attention_mask[: self.tra_max_length], mask]), dtype=torch.bool)
            ...
            ids[_mask_idx] = self.mask_token_id
            mask[_mask_idx] = False
            return torch.tensor(np.hstack([ids, input_ids[self.tra_max_length :]]), dtype=torch.int64), torch.tensor(np.hstack([mask, attention_mask[self.tra_max_length :]]), dtype=torch.bool)
    ...
    def __call__(self, input_ids, attention_mask) -> torch.Tensor:
        if len(input_ids.shape) == 1:
            ...
            for i,a in zip(input_ids, attention_mask):
                ret.append(self._rand_mask(i, a))
            return torch.vstack(
                list(map(lambda x: x[0], ret))
            ), torch.vstack(
                list(map(lambda x: x[1], ret))
...
class TRABMutator:
    ...
    def _mutate(self, input_ids, attention_mask) -> Tuple[torch.Tensor]:
        ...
        if mask_tr < self.mutate_trb_probability:
            ...
            ids = np.array(input_ids[self.tra_max_length:])
            mask = np.array(attention_mask[self.tra_max_length:])
            seq_len = max(map(lambda x: x[0], filter(lambda z: z[1], enumerate(mask))))

            mutation_idx = list(map(lambda z: z[0], list(filter(lambda x: (_AMINO_ACIDS_INDEX_REVERSE.get(x[1][0], None) in _AMINO_ACIDS if x[1][0] in _AMINO_ACIDS_INDEX_REVERSE.keys() else False) and x[1][1], enumerate(zip(ids, mask))))))
            if not self.is_full_length:
                # We focus on the middle region of the CDR3 region
                mutation_idx = list(filter(lambda x: x > 3 and x < seq_len - 3, mutation_idx))

            if random.random() < self.substitution_probability:
                mutation_idx_substitution = np.random.choice(mutation_idx, min(len(mutation_idx), self.max_mutation_aa))
                for i in mutation_idx_substitution:
                    ids[i] = _AMINO_ACIDS_INDEX[np.random.choice(_AMINO_ACIDS, 1)[0]]
            else: 
                if random.random() < self.insertion_probability:
                    mutation_idx_insertion = np.random.choice(mutation_idx, min(len(mutation_idx), self.max_insertion_aa))
                    for i in mutation_idx_insertion:
                        ids = np.insert(ids, i, _AMINO_ACIDS_INDEX[np.random.choice(_AMINO_ACIDS, 1)[0]])[:-1]

                if random.random() < self.deletion_probability:
                    mutation_idx_deletion = np.random.choice(mutation_idx, min(len(mutation_idx), self.max_deletion_aa))
                    for i in mutation_idx_deletion:
                        ids = np.hstack([np.delete(ids, i), np.array([_AMINO_ACIDS_INDEX[_AMINO_ACIDS_ADDITIONALS["PAD"]]])])
            ...
            mutation_idx = list(map(lambda z: z[0], list(filter(lambda x: (_AMINO_ACIDS_INDEX_REVERSE.get(x[1][0], None) in _AMINO_ACIDS if x[1][0] in _AMINO_ACIDS_INDEX_REVERSE.keys() else False) and x[1][1], enumerate(zip(ids, mask))))))
            if not self.is_full_length:
                mutation_idx = list(filter(lambda x: x > 2 and x < seq_len - 3, mutation_idx))
            mutation_idx = np.random.choice(mutation_idx, min(len(mutation_idx), self.max_mutation_aa))
